...
private class AppBarElevationController(private val activityBar: ActivityBarLayout) {
    ...
    set(value) {
      ...
      animator =
        if (value) {
            ...
            ValueAnimator.ofFloat(activityBar.elevation, 0f)
          }
          .apply {
            addUpdateListener { activityBar.elevation = it.animatedValue as Float }
...
fun ObservableScrollView.bindAppBarElevation(activityBar: ActivityBarLayout) {
  ...
  addOnScrollChangedListener { view, _, _, _, _ -> controller.elevated = !view.isTop }
}
