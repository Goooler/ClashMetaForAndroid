...
fun <T> PreferenceScreen.selectableList(
  ...
  launch(Dispatchers.Main) {
    val initial = withContext(Dispatchers.IO) { value.get() }

    impl.selected = values.indexOf(initial)

    impl.clicked { popupSelectMenu(impl, value, valuesText.map { context.getText(it) }, values) }
  }
...
private fun <T> PreferenceScreen.popupSelectMenu(
  ...
  ListPopupWindow(context).apply {
    val adapter = PopupListAdapter(context, valuesText, impl.selected)
    ...
    width =
      adapter.measureWidth(context).coerceAtLeast(context.getPixels(R.dimen.dialog_menu_min_width))

    isModal = true

    horizontalOffset =
      context.getPixels(R.dimen.item_header_component_size) +
