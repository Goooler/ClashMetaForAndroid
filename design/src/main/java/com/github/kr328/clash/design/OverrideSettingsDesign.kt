...
class OverrideSettingsDesign(context: Context, configuration: ConfigurationOverride) :
  ...
  override val root: View by composeView {
    MihomoTheme { OverrideSettingsScreen(configuration = configuration, onReset = ::requestClear) }
  }
    ...
    ProvidePreferenceLocals {
      ...
      LazyColumn(modifier = Modifier.fillMaxSize(), contentPadding = innerPadding) {
        ...
        item(key = "allowLan", contentType = "ListPreference") {
          val state =
            rememberWriteThroughState(configuration.allowLan) { configuration.allowLan = it }
          val value by state
          ListPreference(
            state = state,
        ...
        item(key = "bindAddress", contentType = "EditTextPreference") {
          OverrideEditTextPreferenceItem(
            ...
            state =
              rememberWriteThroughState(configuration.bindAddress) {
        ...
        item(key = "mode", contentType = "ListPreference") {
          ...
          ListPreference(
            state = state,
            values =
              listOf(null, TunnelState.Mode.Direct, TunnelState.Mode.Global, TunnelState.Mode.Rule),
            modifier = Modifier.fillMaxWidth(),
        ...
        item(key = "logLevel", contentType = "ListPreference") {
          val state =
            rememberWriteThroughState(configuration.logLevel) { configuration.logLevel = it }
          val value by state
          ListPreference(
            state = state,
            values =
        ...
        item(key = "dnsPreferH3", contentType = "ListPreference") {
          val state =
            rememberWriteThroughState(configuration.dns.preferH3) {
        ...
        item(key = "dnsListen", contentType = "EditTextPreference") {
          OverrideEditTextPreferenceItem(
            ...
            state =
              rememberWriteThroughState(configuration.dns.listen) { configuration.dns.listen = it },
            placeholder = stringResource(R.string.dont_modify),
            emptyLabel = stringResource(R.string.disabled),
            enabled = dnsEnabled != false,
          )
        ...
        item(key = "dnsIpv6", contentType = "ListPreference") {
          val state =
            rememberWriteThroughState(configuration.dns.ipv6) { configuration.dns.ipv6 = it }
          val value by state
          ListPreference(
            state = state,
            values = booleanOptions,
        ...
        item(key = "dnsUseHosts", contentType = "ListPreference") {
          val state =
            rememberWriteThroughState(configuration.dns.useHosts) {
        ...
        item(key = "dnsFallback", contentType = "EditTextListPreference") {
          OverrideEditTextListPreferenceItem(
            ...
            state =
              rememberWriteThroughState(configuration.dns.fallback) {
                configuration.dns.fallback = it
              },
            placeholder = stringResource(R.string.dont_modify),
            enabled = dnsEnabled != false,
          )
        ...
        item(key = "dnsFakeIpFilterMode", contentType = "ListPreference") {
          ...
          ListPreference(
            ...
            valueToText = { v: ConfigurationOverride.FilterMode? ->
              AnnotatedString(filterModeText(v))
            },
          )
        }
...
private fun OverrideEditTextPreferenceItem(
  ...
  val summary = text?.let { if (it.isEmpty()) emptyLabel else it } ?: placeholder
  Preference(
    modifier = Modifier.fillMaxWidth(),
    title = { Text(title) },
