    ...
    val screen =
      preferenceScreen(context) {
        ...
        val vpn =
          switch(
            ...
            listener = OnChangedListener {
              vpnDependencies.forEach { it.enabled = uiStore.enableVpn }
            }
          }
        ...
        selectableList(
          ...
          valuesText =
            arrayOf(R.string.tun_stack_system, R.string.tun_stack_gvisor, R.string.tun_stack_mixed),
        ...
        clickable(
          ...
          clicked { requests.trySend(Request.StartAccessControlList) }
        }

        if (running) {
          vpn.enabled = false

          vpnDependencies.forEach { it.enabled = false }
        } else {
