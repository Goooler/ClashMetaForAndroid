...
class NewProfileActivity : BaseActivity<NewProfileDesign>() {
  ...
  private suspend fun queryProfileProviders(): List<ProfileProvider> {
    return withContext(Dispatchers.IO) {
      ...
      listOf(ProfileProvider.File(self), ProfileProvider.Url(self), ProfileProvider.QR(self)) +
        providers
    }
  }
  ...
  private suspend fun createProfileByQrCode(url: String) {
    withProfile {
      launchProperties(create(type = Profile.Type.Url, name = getString(R.string.new_profile), url))
    }
  }
}
