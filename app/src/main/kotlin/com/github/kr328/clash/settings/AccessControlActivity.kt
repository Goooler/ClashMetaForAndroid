...
class AccessControlActivity : DesignActivity<AccessControlDesign>() {
  override suspend fun main() {
    ...
    val selected = withContext(Dispatchers.IO) { service.accessControlPackages.toMutableSet() }
        ...
        design.requests.onReceive {
          when (it) {
            ...
            AccessControlDesign.Request.SelectAll -> {
              val all = withContext(Dispatchers.Default) { design.apps.map(AppInfo::packageName) }
