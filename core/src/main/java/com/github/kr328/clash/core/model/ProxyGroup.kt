...
data class ProxyGroup(val type: Proxy.Type, val proxies: List<Proxy>, val now: String) :
    Parcelable {
    class SliceProxyList(private val data: List<Proxy>) : List<Proxy> by data, Parcelable {
        constructor(parcel: Parcel) : this(Proxy.CREATOR.createListFromParcelSlice(parcel, 0, 50))
        ...
        override fun writeToParcel(dest: Parcel, flags: Int) {
            data.writeToParcelSlice(dest, flags)
        }
