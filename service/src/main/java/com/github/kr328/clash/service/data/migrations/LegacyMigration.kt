    ...
    try {
        SQLiteDatabase.openDatabase(file.absolutePath, null, SQLiteDatabase.OPEN_READONLY).use { db
            ...
            when (v) {
                ...
                2,
                3,
                4 -> migrationFromLegacy234(context, db, v)
            }
        }
    } catch (e: Exception) {
...
private suspend fun migrationFromLegacy234(context: Context, legacy: SQLiteDatabase, version: Int) {
    legacy
        .query(
            "profiles",
            arrayOf(
                ...
                "type",
                "uri",
                if (version == 2) "update_interval" else "interval",
            ),
            null,
            null,
            null,
        ...
        .use { cursor ->
            ...
            val interval =
                cursor.getColumnIndex(if (version == 2) "update_interval" else "interval")
...
private suspend fun migrationFromLegacy1(context: Context, legacy: SQLiteDatabase) {
    legacy
        .query("profiles", arrayOf("name", "token", "id", "file"), null, null, null, null, "id")
        .use { cursor ->
            val name = cursor.getColumnIndex("name")
